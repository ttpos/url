...
const createUrlSchema = z.object({
  url: z.string().url(),
  customCode: z.string().regex(/^[\w-]+$/).optional(),
...
async function fetchOpenGraphMetadata(url: string, c: Context): Promise<{
  ogTitle: string
...
app.get('/:shortCode', async (c) => {
  ...
  try {
    ...
    await c.env.URL_ANALYTICS.put(
      shortCode,
      (Number.parseInt(clickCount || '0') + 1).toString(),
    )
...
app.get('/api/analytics/:shortCode', async (c) => {
  const shortCode = c.req.param('shortCode')
  const clickCount = await c.env.URL_ANALYTICS.get(shortCode)
  return c.json({ shortCode, clickCount: clickCount ? Number.parseInt(clickCount) : 0 })
})
