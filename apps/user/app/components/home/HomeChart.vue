...
const { data } = await useAsyncData<any[]>(async () => {
  ...
  return dates.map((date: any) => ({ date, amount: Math.floor(Math.random() * (max - min + 1)) + min }))
}, {
...
const template = (d: any) => `${formatDate(d.date)}: ${formatNumber(d.amount)}`
</script>
